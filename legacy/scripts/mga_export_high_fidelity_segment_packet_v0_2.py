#!/usr/bin/env python3
"""
mga_export_high_fidelity_segment_packet_v0_2.py

Build an explicit high-fidelity handoff segment packet directly from the stitched
patch-corrector JSONL (mga_stitched_patch_corrector_v0_1.py). This avoids the
V0.1 schema/provenance trap where finalize packets may not embed the full source
state tree.
...
SCHEMA_VERSION = "mga_high_fidelity_segment_packet.v0.2"
...
def make_packet(corrected: Mapping[str, Any], bplanes: Sequence[Mapping[str, Any]], spice: Any, catalog: Mapping[str, Any], args: argparse.Namespace, rank: int) -> Dict[str, Any]:
    ...
    if bp is None:
        failures.append("missing_matching_bplane_packet_for_departure_velocity")
...
def main() -> int:
    ...
    print("="*80)
    print("MGA HIGH-FIDELITY SEGMENT PACKET EXPORT V0.2")
    print("="*80)
