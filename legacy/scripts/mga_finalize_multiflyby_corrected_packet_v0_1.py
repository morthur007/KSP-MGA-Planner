#!/usr/bin/env python3
"""
MGA Finalize Multi-Flyby Corrected Route Packet V0.1

Consolidates the output of mga_multiflyby_patch_corrector_v0_2.py into a
route-level manifest. This stage intentionally distinguishes:
...
def extract_segments(rec: Dict[str, Any]) -> List[Dict[str, Any]]:
    segs = get_path(rec, "segments", "segment_results", "corrected_segments", default=None)
...
def extract_flybys(rec: Dict[str, Any]) -> List[Dict[str, Any]]:
    for path in (
        "flybys", "local_flybys", "stitched_flybys", "source.flybys",
        "source_stitched_packet.flybys", "stitched_packet.flybys",
    ):
        val = get_path(rec, path, default=None)
        if isinstance(val, list):
            return val
    # Try source embedded in corrected records.
    src = rec.get("source") or rec.get("source_record") or rec.get("source_stitched_packet")
...
def flyby_summary(flybys: List[Dict[str, Any]]) -> Tuple[str, Optional[float], Optional[float], Optional[float]]:
    ...
    for f in flybys:
        ...
        rp = as_float(get_path(f, "rp_margin_km", "periapsis.rp_margin_km", "rp_margin", default=None))
        if rp is not None:
            rp_margins.append(rp)
        vm = as_float(get_path(f, "vinf_mismatch_m_s", "v_inf_mismatch_m_s", "vinf_mis_m_s", default=None))
        if vm is not None:
            vinf_mis.append(vm)
        alt = as_float(get_path(f, "periapsis_altitude_km", "altitude_km", "periapsis.altitude_km", default=None))
        if alt is not None:
            alts.append(alt)
...
def build_manifest(rec: Dict[str, Any], args: argparse.Namespace, idx: int) -> Dict[str, Any]:
    ...
    patch_dv = as_float(get_path(rec, "total_correction_m_s", "total_patch_correction_m_s", "known_correction_m_s", default=None))
    if patch_dv is None:
        patch_dv = sum_from_segments(segs, ["correction_m_s", "dv_m_s", "delta_v_m_s", "correction_norm_m_s"]) or 0.0

    max_pos_after = as_float(get_path(rec, "max_position_miss_after_km", "max_miss_after_km", default=None))
    if max_pos_after is None:
        max_pos_after = max_from_segments(segs, ["position_miss_after_km", "miss_after_km", "final_position_miss_km"]) or float("inf")
    ...
    pass_source = as_bool(get_path(rec, "pass_correction", "pass", "pass_manifest", default=False))
    pass_manifest = bool(
        pass_source
    ...
    out = {
        "packet_id": rec.get("correction_id") or rec.get("id") or f"mf_final_{idx:04d}",
...
def main() -> int:
    ...
    print("=" * 80)
    print("MGA FINALIZE MULTI-FLYBY CORRECTED PACKET V0.1")
    print("=" * 80)
