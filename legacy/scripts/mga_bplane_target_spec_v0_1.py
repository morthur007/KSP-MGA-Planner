#!/usr/bin/env python3
"""
mga_bplane_target_spec_v0_1.py
...
SCHEMA_VERSION = "mga_bplane_target_spec.v0.1"
...
def main() -> int:
    ...
    print("=" * 80)
    print("MGA B-PLANE TARGET SPEC V0.1")
    print("=" * 80)
    ...
    for i, s in enumerate(specs[:10], start=1):
        fb = (s.get("flyby_targets") or [{}])[0]
        hyp = fb.get("hyperbola") if isinstance(fb, Mapping) else {}
        print(
            f" {i}. {s.get('sequence')} | score={finite(s.get('score')):.3f} | "
            f"corr={finite(s.get('total_departure_correction_m_s')):.3f} m/s | "
            f"rp_margin={finite(s.get('min_rp_margin_km')):.1f} km | "
            f"alt={finite(hyp.get('periapsis_altitude_km')):.1f} km | "
            f"B.T={finite(fb.get('b_dot_t_km')):.1f} km | B.R={finite(fb.get('b_dot_r_km')):.1f} km"
        )
    print("=" * 80)
