...
Example
-------
  python mga_spice_nbody_validate_arcs_v0_2.py \
...
SCHEMA_VERSION = "mga_spice_nbody_validate_arcs.v0.2"
...
def _validate_route_worker(payload: Tuple[Dict[str, Any], Dict[str, Any]]) -> Dict[str, Any]:
    ...
    dynamics_mode = str(_WORKER_CONFIG.get("dynamics_mode", "patched_heliocentric"))
    ...
    for i, leg in enumerate(legs):
        ...
        # Important: these Lambert legs are heliocentric patched-conic arcs whose
        # endpoints are body centers. A full point-mass N-body integration starting
        # exactly at a planet center is singular/unphysical. In the default
        # patched_heliocentric mode we therefore exclude the current leg's origin
        # and target bodies from the perturbing set. Use full_nbody only for
        # trajectories initialized at a real escape/arrival patch point, not at
        # a body center.
        if dynamics_mode == "two_body":
            leg_perturbers: List[BodyMu] = []
        elif dynamics_mode == "full_nbody":
            leg_perturbers = perturbers
        elif dynamics_mode == "patched_heliocentric":
            endpoint_names = {origin, target}
            leg_perturbers = [b for b in perturbers if b.name not in endpoint_names]
        else:
        ...
        try:
            sol = scipy_integrate.solve_ivp(
                lambda t, y: _accel_central_frame(t, y, central, frame, mu_c, leg_perturbers),
                (depart_et, arrive_et),
                y0,
        ...
        leg_results.append(LegValidation(
            ...
            notes=(f"dynamics_mode={dynamics_mode}; perturbers={len(leg_perturbers)}; " + str(getattr(sol, "message", ""))),
        ))
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    p.add_argument("--dynamics-mode", default="patched_heliocentric", choices=["two_body", "patched_heliocentric", "full_nbody"], help="two_body: Sun only sanity check; patched_heliocentric: Sun plus third bodies excluding each leg endpoint bodies; full_nbody: includes endpoint bodies, only valid for non-center patch states.")
    ...
    print("=" * 80)
    print("MGA SPICE N-BODY ARC VALIDATION V0.2")
    print("=" * 80)
