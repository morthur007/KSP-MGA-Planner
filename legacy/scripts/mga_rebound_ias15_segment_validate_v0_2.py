...
V0.2 fixes the comparison frame for heliocentric patched segments: REBOUND
particles evolve in an inertial frame where the central body is free to move under
third-body gravity. Packet endpoints, however, are heliocentric states. Therefore
absolute packet endpoints must be compared against spacecraft-minus-central, not
against the raw spacecraft particle coordinates.
...
SCHEMA_VERSION = "mga_rebound_ias15_segment_validation.v0.2"
...
def validate_heliocentric_segment(seg: Mapping[str, Any], catalog: Mapping[str, Any]) -> Dict[str, Any]:
    ...
    if mode == "endpoint_bodies" and target_body in index:
        ...
        # Packet heliocentric states are expressed relative to the central body.
        # In REBOUND the central body is a massive particle and is not fixed when
        # perturbing bodies are included. Compare in the same heliocentric frame
        # by subtracting the integrated central-body state at the endpoint.
        ...
        compare_mode = "central_relative_packet_endpoint"
    return {
        "ok": True,
        ...
        "central_body_endpoint_drift_km": vnorm(central_r) if 'central_r' in locals() else 0.0,
        "central_body_endpoint_speed_m_s": vnorm(central_v)*1000.0 if 'central_v' in locals() else 0.0,
    }
...
def main() -> int:
    ...
    print("="*80)
    print("MGA REBOUND IAS15 SEGMENT VALIDATION V0.2")
    print("="*80)
