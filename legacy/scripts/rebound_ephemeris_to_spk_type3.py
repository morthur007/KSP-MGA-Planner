...
def fit_type3_record(
    ...
    """
    Fit one SPK Type 3 logical record.

    values shape: (n_samples, 6), units km and km/s.
    Record layout required by SPK Type 3:
      MID, RADIUS, X coeffs, Y coeffs, Z coeffs, VX coeffs, VY coeffs, VZ coeffs
    """
    record: List[float] = []  # <--- CORRIGIDO
    max_resid = 0.0
...
def _worker_fit_record(values_3d: np.ndarray, taus: np.ndarray, cheby_degree: int, mid_et: float, radius_s: float) -> List[Tuple[List[float], float]]:
    """
    Processo trabalhador isolado que roda em paralelo.
    Recebe um cubo Numpy com as posições de todos os corpos em 1 record e calcula os polinômios.
    """
    results = []
    n_bodies = values_3d.shape[0]
    for i in range(n_bodies):
        # Chama a função fit_type3_record que já existe no seu script
        record, max_resid = fit_type3_record(taus, values_3d[i], cheby_degree, mid_et, radius_s)
        results.append((record, max_resid))
    return results
...
def build_chebyshev_records(
    ...
    # Inicia o Pool de Processos. Ele detecta automaticamente quantos núcleos seu CPU tem.
    with concurrent.futures.ProcessPoolExecutor() as executor:
        for rec in range(n_records):
            rec_start_rel = rec * record_span_s
            rec_end_rel = min((rec + 1) * record_span_s, duration_s)
            actual_span_s = rec_end_rel - rec_start_rel
            mid_rel = 0.5 * (rec_start_rel + rec_end_rel)
            radius_s = 0.5 * actual_span_s
            mid_et = start_et + mid_rel

            # Pré-aloca uma matriz NumPy 3D para guardar os dados brutos fisicos (Corpos x Amostras x 6_Eixos)
            values_3d = np.empty((len(target_names), fit_samples, 6), dtype=float)

            for step_idx, tau in enumerate(taus):
                ...
                # Extrai posições para o bloco 3D
                for body_idx, name in enumerate(target_names):
                    values_3d[body_idx, step_idx, :] = particle_state_relative_km(
                        sim, name_to_index[name], center_index
                    )

            # Envia a matriz bruta 3D para um Worker em outro núcleo do CPU fazer a matemática
            future = executor.submit(
                _worker_fit_record, values_3d, taus, config.cheby_degree, mid_et, radius_s
            )
            ...
            if output_report_every > 0 and ((rec + 1) % output_report_every == 0 or rec == n_records - 1):
                ...
                print(f"[REBOUND FÍSICA] Record {rec+1}/{n_records} enviado pra fila; sim_t={sim.t / JULIAN_YEAR_S:.3f} yr; wall={elapsed:.1f}s", flush=True)

        print("[MULTIPROCESSING] Integração N-Body concluída. Aguardando workers finalizarem a matemática...", flush=True)

        # Coleta os resultados matemáticos garantindo que a ordem cronológica do arquivo .bsp seja mantida
        for rec, future in enumerate(futures):
            worker_results = future.result()  # Bloqueia até esse registro específico ficar pronto
            
            for body_idx, name in enumerate(target_names):
                record, max_resid = worker_results[body_idx]
                cdata_by_body[name].extend(record)
                residuals[name].append(max_resid)
            
            if output_report_every > 0 and ((rec + 1) % output_report_every == 0 or rec == n_records - 1):
                print(f"[REBOUND MATEMÁTICA] Escrito {rec+1}/{n_records} records no buffer...", flush=True)
