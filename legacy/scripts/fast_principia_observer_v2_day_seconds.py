...
def main() -> int:
    ...
    ap.add_argument("--duration-days", type=float, default=90.0, help="Duration in command-defined days. Use --day-seconds to choose what a day means.")
    ap.add_argument("--day-seconds", type=float, default=86400.0, help="Seconds per --duration-days unit. Default 86400 SI seconds. For JNSQ/KSP calendar days, use the in-game day length, e.g. 43200 if the clock day is 12h.")
    ap.add_argument("--duration-seconds", type=float, default=0.0, help="If >0, overrides --duration-days*--day-seconds and sets exact UT duration in seconds.")
    ...
    try:
        ...
        duration_seconds = float(args.duration_seconds) if args.duration_seconds and args.duration_seconds > 0 else float(args.duration_days) * float(args.day_seconds)
        end_ut = start_ut + duration_seconds
        target_ut = start_ut
        next_sample_index = 0
            ...
            while True:
                ...
                if actual_ut >= target_ut:
                    ...
                    if now_real - last_print > 2.0:
                        ...
                        print(
                            f"[{next_sample_index+1}] UT={actual_ut:.2f} day={(actual_ut-start_ut)/args.day_seconds:.2f}/"
                            f"{duration_seconds/args.day_seconds:.2f} lag={actual_ut-target_ut:.2f}s ok={ok} failed={failed} "
