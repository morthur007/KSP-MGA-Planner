#!/usr/bin/env python3
"""
mga_export_high_fidelity_segment_packet_v0_3.py
...
V0.3 change: if the original B-plane packet cannot be matched to recover the
pre-flyby departure velocity, reconstruct the base departure velocity by solving
a Sun-centred Lambert arc from the origin centre at depart_et to the flyby body
centre at periapsis_et, then apply the pre-flyby correction vector. This removes
the fragile dependence on B-plane packet IDs while preserving the stitched
corrector semantics.
...
SCHEMA_VERSION = "mga_high_fidelity_segment_packet.v0.3"
...
def matching_bplane(corrected: Mapping[str, Any], bplanes: Sequence[Mapping[str, Any]]) -> Optional[Mapping[str, Any]]:
    keys: List[str] = []
    for obj in (corrected, corrected.get("source_stitched_packet") if isinstance(corrected.get("source_stitched_packet"), Mapping) else {},
                corrected.get("source_stitched_packet", {}).get("target") if isinstance(corrected.get("source_stitched_packet"), Mapping) and isinstance(corrected.get("source_stitched_packet", {}).get("target"), Mapping) else {}):
        if isinstance(obj, Mapping):
            for k in ("packet_id", "route_id", "correction_id", "closure_id", "stitched_packet_id"):
                val = obj.get(k)
                if val is not None:
                    keys.append(str(val))
    for bp in bplanes:
        for k in ("packet_id", "route_id", "correction_id", "closure_id"):
            val = bp.get(k)
            if val is not None and str(val) in keys:
                return bp
    # Fallback: exact sequence only when there is a single candidate for that sequence.
    seq = str(corrected.get("sequence") or "")
    cands = [bp for bp in bplanes if str(bp.get("sequence") or "") == seq]
    if len(cands) == 1:
        return cands[0]
    return None
...
def main() -> int:
    ...
    print("="*80)
    print("MGA HIGH-FIDELITY SEGMENT PACKET EXPORT V0.3")
    print("="*80)
