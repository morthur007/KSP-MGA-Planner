#!/usr/bin/env python3
"""
mga_multiflyby_patch_corrector_v0_2.py
...
SCHEMA_VERSION = "mga_multiflyby_patch_corrector.v0.2"
...
def _velocity_required_for_segment(seg: Mapping[str, Any], cfg: Mapping[str, Any]) -> bool:
    """Return whether endpoint velocity should be a hard pass criterion.

    The patch corrector solves only a 3-variable position targeting problem
    (the small impulse at the beginning of the heliocentric segment). A 6D
    position+velocity match is over-constrained unless we also free epoch, a
    second impulse, or local B-plane parameters. Therefore, velocity should be
    treated as a diagnostic by default for this stage.

    Modes:
      none:             never use velocity as a hard pass criterion.
      final_exempt:     require velocity except for final body-center arrival.
      patch_entries:    require velocity only when target is a *_soi_in patch.
      all:              require velocity for every segment.
    """
    mode = str(cfg.get("velocity_pass_mode") or "none").lower()
    if mode in ("none", "diagnostic", "position_only"):
        return False
    kind = str(seg.get("segment_kind") or "")
    target = str(seg.get("target_label") or "")
    if mode in ("final_exempt", "final_target_exempt"):
        return kind != "last_flyby_exit_to_final_target" and not target.endswith("_center")
    if mode in ("patch_entries", "patch_points"):
        return target.endswith("_soi_in")
    if mode == "all":
        return True
    return False
...
def _correct_segment(seg: Mapping[str, Any], all_perturbers: Sequence[BodyMu], cfg: Mapping[str, Any]) -> SegmentCorrection:
    ...
    velocity_required = _velocity_required_for_segment(seg, cfg)
    pass_vel = (
        bool(ok1 and math.isfinite(vel_mis) and vel_mis <= vel_threshold_m_s)
        if velocity_required else True
    )
    return SegmentCorrection(
        ...
        notes=(
            "ok" if (ok1 and pass_pos and pass_vel)
            else (f"position_fail:{miss1}" if ok1 and not pass_pos else f"velocity_diagnostic_or_fail:{vel_mis}")
        ) if ok1 else msg1,
    )
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        "--velocity-pass-mode",
        default="none",
        choices=["none", "final_exempt", "patch_entries", "all"],
        help="Whether endpoint velocity is a hard pass criterion. Default 'none' treats velocity as diagnostic because this stage solves a 3-variable position correction.",
    )
...
def main() -> int:
    ...
    cfg = {
        ...
        "velocity_pass_mode": args.velocity_pass_mode,
    ...
    print("="*80)
    print("MGA MULTI-FLYBY PATCH CORRECTOR V0.2")
    print("="*80)
    ...
    print(f"Velocity pass mode:{args.velocity_pass_mode}")
