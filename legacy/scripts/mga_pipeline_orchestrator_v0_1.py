#!/usr/bin/env python3
"""
MGA Pipeline Orchestrator V0.1
...
def grav_bodies(cfg: Dict[str, Any]) -> List[str]:
    return list(cfg["bodies"].get("gravitating_bodies", cfg["bodies"]["search_bodies"]))
...
def build_commands(cfg: Dict[str, Any], stages: Optional[set[str]] = None) -> List[Cmd]:
    ...
    if want("legs"):
        for origin in bodies_cfg["search_bodies"]:
            targets = [b for b in bodies_cfg["search_bodies"] if b != origin]
    ...
    if want("beam"):
        argv = ["python", script("mga_beam_search_v0_3.py"),
                "--input-jsonl", str(legs_dir / "*all_seeds.jsonl"),
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    if args.plan:
        print("=" * 88)
        print("MGA PIPELINE ORCHESTRATOR V0.1 — PLAN")
    ...
    if args.run:
        for cmd in cmds:
            run_cmd(cmd, dry_run=False, continue_on_error=args.continue_on_error)
