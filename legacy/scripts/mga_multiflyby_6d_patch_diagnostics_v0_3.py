#!/usr/bin/env python3
"""
MGA Multi-Flyby 6D Patch Diagnostics V0.3
...
def sanitize_json(obj: Any) -> Any:
    """Recursively convert non-JSON-safe values to stable JSON values.

    Python's json encoder does not call default= for builtin floats, so NaN/Inf
    must be removed before json.dumps(..., allow_nan=False).
    """
    if isinstance(obj, float):
        return obj if math.isfinite(obj) else None
...
def write_json(path: Path, payload: Mapping[str, Any]) -> None:
    ...
    with path.open("w", encoding="utf-8") as f:
        json.dump(sanitize_json(payload), f, indent=2, ensure_ascii=False, allow_nan=False, default=json_default)
        f.write("\n")
...
def write_jsonl(path: Path, rows: Sequence[Mapping[str, Any]]) -> None:
    ...
    with path.open("w", encoding="utf-8") as f:
        for r in rows:
            f.write(json.dumps(sanitize_json(r), ensure_ascii=False, allow_nan=False, separators=(",", ":"), default=json_default))
            f.write("\n")
...
def write_csv(path: Path, rows: Sequence[Mapping[str, Any]]) -> None:
    fields = [
        ...
        "final_local_speed_km_s", "target_local_speed_km_s", "target_body_speed_km_s", "target_local_source",
...
def main() -> int:
    ...
    for ridx, rec in enumerate(records, start=1):
        ...
        for seg in segments:
            ...
            # Prefer explicit patch states when present, but do not depend on them.
            # The segment definition already contains the central-frame target velocity.
            # For *_soi_in/out targets we can reconstruct the local expected velocity as:
            #     v_target_local = v_target_central - v_body_spice(t1)
            # This avoids NaN when the embedded source packet schema differs.
            if patch_state:
                ...
                if loc_v is not None:
                    ...
                    target_local_source = "embedded_patch_state"

            # Fallback for patch-point targets: derive local target velocity from SPICE body state.
            if body_v is None and patch_body and patch_kind is not None:
                try:
                    _r_b, body_v2 = corr._state_body(patch_body, t1, args.central_body, args.frame)  # type: ignore[attr-defined]
                    body_v = body_v2
                except Exception:
                    body_v = None
            if target_local_v is None and body_v is not None and patch_body and patch_kind is not None:
                target_local_v = vsub(v_target, body_v)
                target_local_source = "segment_target_minus_spice_body"

            # Final body-center arrival: local target velocity is zero by definition,
            # but nonzero arrival v_inf is expected and classified separately.
            if body_v is None and target_label.endswith("_center"):
                ...
                try:
                    ...
                    body_v = None
            if target_local_v is None and body_v is not None and target_label.endswith("_center"):
                target_local_v = (0.0, 0.0, 0.0)
                target_local_source = "body_center_zero_local_velocity"
            ...
            row: Dict[str, Any] = {
                ...
                "target_local_source": target_local_source,
    ...
    summary = {
        "schema_version": "mga_multiflyby_6d_patch_diagnostics.v0.3",
    ...
    print("="*80)
    print("MGA MULTI-FLYBY 6D PATCH DIAGNOSTICS V0.3")
    print("="*80)
