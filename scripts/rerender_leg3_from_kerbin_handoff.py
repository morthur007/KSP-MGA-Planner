...
def source_costs(row: dict[str, Any]) -> dict[str, float]:
    src = row.get("source_leg2_rerender") if isinstance(row.get("source_leg2_rerender"), dict) else {}
    return {
        "leg1_dv0_norm_m_s": safe_float(row.get("leg1_dv0_norm_m_s"), safe_float(src.get("leg1_dv0_norm_m_s"), 0.0)),
        "leg1_dsm_norm_m_s": safe_float(row.get("leg1_dsm_norm_m_s"), safe_float(src.get("leg1_dsm_norm_m_s"), 0.0)),
        "eve_powered_lower_bound_m_s": safe_float(row.get("eve_powered_lower_bound_m_s"), safe_float(src.get("eve_powered_lower_bound_m_s"), 0.0)),
        "leg2_dsm_norm_m_s": safe_float(row.get("leg2_dsm_norm_m_s"), safe_float(src.get("leg2_dsm_norm_m_s"), 0.0)),
        "total_with_departure_before_leg3_m_s": safe_float(row.get("total_with_departure_dv_m_s"), safe_float(src.get("total_with_departure_dv_m_s"), 0.0)),
        "total_post_departure_before_leg3_m_s": safe_float(row.get("total_post_departure_dv_m_s"), safe_float(src.get("total_post_departure_dv_m_s"), 0.0)),
    }
...
def make_start_state(row: dict[str, Any], mode: str, cfg: dict[str, Any]) -> tuple[np.ndarray, np.ndarray, float, float, list[str]]:
    ...
    Modes:
      ref_powered: use outgoing v∞ vector from Kerbin handoff audit; cost is powered_lower_bound_m_s.
      same_mag_ref_dir: reference direction, incoming magnitude; cost 0 if turn feasible, useful as ballistic-ish branch.
    """
    ...
    if mode == "ref_powered":
        ...
        notes.append("vinf_out_ref_from_kerbin_audit")
...
def solve_dsm_to_target_position(
    ...
    row_idx = int(handoff.get("row_index", handoff.get("handoff_row_index", 0)) or 0)
    r_start, v_start, t_start, kerbin_powered_cost, notes = make_start_state(handoff, mode, cfg)
    ...
    r_dsm, v_dsm, st, msg = propagate_state(
        srv,
        req_id=f"leg3_pre_dsm_{os.getpid()}_{dep_id}_{row_idx}_{mode}_{t_arr:.1f}_{dsm_fraction:.3f}",
        t0=t_start,
        t1=t_dsm,
        r0=r_start,
        v0=v_start,
    ...
    for it in range(int(cfg["iterations"])):
        base_r, base_v, st, msg = propagate_state(
            srv,
            req_id=f"leg3_base_{os.getpid()}_{dep_id}_{row_idx}_{mode}_{it}_{t_arr:.1f}_{dsm_fraction:.3f}",
            t0=t_dsm,
            t1=t_arr,
            r0=r_dsm,
            v0=v_dsm + dsm,
        ...
        for j in range(3):
            ...
            rp, vp, stp, msgp = propagate_state(
                srv,
                req_id=f"leg3_fd_{os.getpid()}_{dep_id}_{row_idx}_{mode}_{it}_{j}_{t_arr:.1f}_{dsm_fraction:.3f}",
                t0=t_dsm,
                t1=t_arr,
                r0=r_dsm,
                v0=v_dsm + dsm + dd,
        ...
        try:
            last_cond = float(np.linalg.cond(J))
    ...
    final_r, final_v, st, msg = propagate_state(
        srv,
        req_id=f"leg3_final_{os.getpid()}_{dep_id}_{row_idx}_{mode}_{t_arr:.1f}_{dsm_fraction:.3f}",
        t0=t_dsm,
        t1=t_arr,
        r0=r_dsm,
        v0=v_dsm + dsm,
    ...
    dsm_norm = norm(dsm)
    costs = source_costs(handoff)

    prior_post = costs["total_post_departure_before_leg3_m_s"]
    if not math.isfinite(prior_post) or prior_post <= 0:
        prior_post = costs["leg1_dsm_norm_m_s"] + costs["eve_powered_lower_bound_m_s"] + costs["leg2_dsm_norm_m_s"]
    prior_all = costs["total_with_departure_before_leg3_m_s"]
    if not math.isfinite(prior_all) or prior_all <= 0:
        prior_all = costs["leg1_dv0_norm_m_s"] + prior_post

    total_after_departure = prior_post + kerbin_powered_cost + dsm_norm
    total_with_departure = prior_all + kerbin_powered_cost + dsm_norm
    ...
    score = (
        ...
        + kerbin_powered_cost / float(cfg["powered_scale_m_s"]) * float(cfg["powered_weight"])
    )
    ...
    if dsm_norm > float(cfg["accept_dsm_m_s"]):
        valid = False; reasons.append("leg3_dsm_too_large")
    if kerbin_powered_cost > float(cfg["accept_powered_m_s"]):
        valid = False; reasons.append("kerbin_powered_too_large")
    if clipped:
        valid = False; reasons.append("leg3_dsm_clipped")
    ...
    return {
        ...
        "t_arr_s": t_arr,
        "dsm_fraction": dsm_fraction,
        "leg3_arrival_offset_days": (t_arr - float(cfg["leg3_t_end_s"])) / 86400.0,
        "kerbin_arrival_offset_days": safe_float(handoff.get("arrival_offset_from_leg3_start_days")),
        "kerbin_powered_lower_bound_m_s": kerbin_powered_cost,
        "leg1_dv0_norm_m_s": costs["leg1_dv0_norm_m_s"],
        "leg1_dsm_norm_m_s": costs["leg1_dsm_norm_m_s"],
        "eve_powered_lower_bound_m_s": costs["eve_powered_lower_bound_m_s"],
        "leg2_dsm_norm_m_s": costs["leg2_dsm_norm_m_s"],
        "leg3_dsm_raw_m_s": dsm.tolist(),
        "leg3_dsm_levela_m_s": raw_to_levela(dsm).tolist(),
        "leg3_dsm_norm_m_s": dsm_norm,
        "leg3_dsm_clipped": clipped,
        "jacobian_condition_estimate": last_cond,
...
def main() -> int:
    ap = argparse.ArgumentParser(description="Re-render leg3 Kerbin->Jool from rerendered Kerbin handoff rows using one DSM.")
    ap.add_argument("--kerbin-handoff-json", type=Path, required=True)
    ...
    ap.add_argument("--leg", type=int, default=3)
    ...
    ap.add_argument("--arr-body", default="JOOL")
    ap.add_argument("--center", default="SUN")
    ap.add_argument("--frame", default="J2000")
    ap.add_argument("--max-handoffs", type=int, default=20)
    ap.add_argument("--statuses", default="PASS,POWERED", help="Comma list of Kerbin handoff statuses to use.")
    ap.add_argument("--outgoing-modes", default="ref_powered,same_mag_ref_dir")
    ap.add_argument("--arrival-offset-days", default="-30,-21,-15,-10,-7,-5,-3,-1,0,1,3,5,7,10,15,21,30")
    ap.add_argument("--dsm-fractions", default="0.03,0.05,0.08,0.10,0.15,0.20,0.35,0.50,0.65")
    ap.add_argument("--min-dsm-after-start-s", type=float, default=7200.0)
    ...
    ap.add_argument("--max-dsm-m-s", type=float, default=4000.0)
    ap.add_argument("--accept-pos-km", type=float, default=200000.0)
    ap.add_argument("--accept-dsm-m-s", type=float, default=1500.0)
    ap.add_argument("--accept-powered-m-s", type=float, default=1500.0)
    ap.add_argument("--pos-scale-km", type=float, default=200000.0)
    ...
    ap.add_argument("--vel-weight", type=float, default=0.03)
    ap.add_argument("--dsm-weight", type=float, default=0.60)
    ap.add_argument("--powered-weight", type=float, default=0.40)
    ap.add_argument("--start-at-actual-arrival-position", action="store_true", help="Use actual leg2 final_r instead of the body centre as leg3 start position.")
    ...
    handoffs = read_handoff_rows(args.kerbin_handoff_json, args.max_handoffs, statuses)
    ...
    print("=== RERENDER LEG3 FROM KERBIN HANDOFF ===")
    ...
    summary = {
        "schema": "leg3_rerender_from_kerbin_handoff.v1",
        "n_rows": len(rows),
        "n_ok": len(ok_rows),
    ...
    (args.output_dir / "leg3_rerender_all.json").write_text(json.dumps(rows, indent=2, default=str) + "\n")
    (args.output_dir / "leg3_rerender_top.json").write_text(json.dumps(ok_rows[:100], indent=2, default=str) + "\n")
    (args.output_dir / "leg3_rerender_result.json").write_text(json.dumps(summary, indent=2, default=str) + "\n")
    write_csv(args.output_dir / "leg3_rerender_summary.csv", ok_rows)

    if best:
        print("\n=== TOP LEG3 RERENDER RESULTS ===")
        print("rank valid mode            dep        arr3_off_d pos_km vel_m_s k_pow l1_dsm e_pow l2_dsm l3_dsm total_post total_all reasons")
        for i, r in enumerate(ok_rows[:20], start=1):
            print(
                ...
                f"{safe_float(r.get('leg3_arrival_offset_days')):10.2f} "
                f"{safe_float(r.get('final_pos_err_km')):8.1f} "
                f"{safe_float(r.get('final_vel_err_m_s')):8.1f} "
                f"{safe_float(r.get('kerbin_powered_lower_bound_m_s')):6.1f} "
                f"{safe_float(r.get('leg1_dsm_norm_m_s')):7.1f} "
                f"{safe_float(r.get('eve_powered_lower_bound_m_s')):6.1f} "
                f"{safe_float(r.get('leg2_dsm_norm_m_s')):7.1f} "
                f"{safe_float(r.get('leg3_dsm_norm_m_s')):7.1f} "
    ...
    print(f"[OK] wrote {args.output_dir / 'leg3_rerender_result.json'}")
    print(f"[OK] wrote {args.output_dir / 'leg3_rerender_summary.csv'}")
