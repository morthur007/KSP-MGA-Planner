...
def main() -> int:
    ...
    dep_body = (args.dep_body or leg.get("dep") or leg.get("dep_body") or active.get("nav_body") or "KERBIN").upper()
    arr_body = (args.arr_body or leg.get("arr") or leg.get("arr_body")).upper()
    ...
    if args.t_arr_s is not None:
        ...
        rk = json.loads(args.rank_json.read_text())
        cand = rk["top"][args.top_index] if "top" in rk else rk.get("candidate", rk)
    ...
    vinf_dep = get_vinf_raw_m_s(leg, "dep")
