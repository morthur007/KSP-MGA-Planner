...
void Plugin::DumpSampledEphemerisToCsv(
    std::string const& path,
    std::string const& central_body_name,
    double const start_offset_s,
    double const duration_s,
    double const step_s) const {
  CHECK_GT(step_s, 0.0);
  CHECK_GE(duration_s, 0.0);

  // Serializa só para obter nomes e μ. 
  serialization::Plugin plugin_message;
  WriteToMessage(&plugin_message);

  auto const& bodies = ephemeris_->bodies();
  CHECK_EQ(bodies.size(), plugin_message.ephemeris().body_size());

  int central_index = -1;
  for (int i = 0; i < plugin_message.ephemeris().body_size(); ++i) {
    if (plugin_message.ephemeris().body(i).name() == central_body_name) {
      central_index = i;
      break;
    }
  }
  CHECK_NE(central_index, -1) << "Central body not found: " << central_body_name;
  ...
  auto transform_position = [](double const x,
                               double const y,
                               double const z,
                               double& out_x,
                               double& out_y,
                               double& out_z) {
    // Principia raw -> LevelA canonical frame:
    // (X, Y, Z) -> (-Y, +Z, +X)
    out_x = -y;
    out_y =  z;
    out_z =  x;
  ...
  out
      ...
      << "raw_vx_m_s,raw_vy_m_s,raw_vz_m_s\n";

  int const sample_count =
      static_cast<int>(std::floor(duration_s / step_s)) + 1;

  for (int sample_index = 0; sample_index < sample_count; ++sample_index) {
    double const offset_s = start_offset_s + sample_index * step_s;
    Instant const t = current_time_ + offset_s * Second;

    ephemeris_->Prolong(t).IgnoreError();

    // 1. CORREÇÃO: O tempo é serializado como um Point
    serialization::Point t_message;
    t.WriteToMessage(&t_message);
    double const epoch_s = t_message.scalar().magnitude();

    std::vector<SampledState> states;
    ...
    for (int i = 0; i < bodies.size(); ++i) {
      ...
      // 2. CORREÇÃO: Extraímos Posição e Velocidade separadamente (igual ao snapshot 1)
      serialization::Point p_message;
      degrees_of_freedom.position().WriteToMessage(&p_message);
      auto const& r = p_message.multivector().vector();

      serialization::Multivector v_message;
      degrees_of_freedom.velocity().WriteToMessage(&v_message);
      auto const& v = v_message.vector();

      SampledState st;
      // 3. CORREÇÃO: Adicionado o .quantity() antes do .magnitude()
      st.raw_x = r.x().quantity().magnitude();
      st.raw_y = r.y().quantity().magnitude();
      st.raw_z = r.z().quantity().magnitude();

      st.raw_vx = v.x().quantity().magnitude();
      st.raw_vy = v.y().quantity().magnitude();
      st.raw_vz = v.z().quantity().magnitude();

      transform_position(st.raw_x, st.raw_y, st.raw_z,
                         st.x, st.y, st.z);
      transform_position(st.raw_vx, st.raw_vy, st.raw_vz,
                         st.vx, st.vy, st.vz);

      states[i] = st;
    }

    SampledState const central = states[central_index];

    for (int i = 0; i < bodies.size(); ++i) {
      auto const& body_message = plugin_message.ephemeris().body(i);
      auto const& st = states[i];

      double x = st.x - central.x;
      double y = st.y - central.y;
      double z = st.z - central.z;

      double vx = st.vx - central.vx;
      double vy = st.vy - central.vy;
      double vz = st.vz - central.vz;
      ...
      out
          ...
          << epoch_s << ","
          << body_message.name() << ","
          << body_message.gravitational_parameter().magnitude() << ","
          << x << "," << y << "," << z << ","
          << vx << "," << vy << "," << vz << ","
          << st.raw_x << "," << st.raw_y << "," << st.raw_z << ","
          << st.raw_vx << "," << st.raw_vy << "," << st.raw_vz
          << "\n";
    }
  }
}
void Plugin::DumpSampledEphemerisToBinaryChunks(
    std::string const& output_dir,
    std::string const& central_body_name,
    double const start_offset_s,
    double const duration_s,
    double const step_s,
    double const chunk_s) const {
  CHECK_GT(step_s, 0.0);
  CHECK_GT(chunk_s, 0.0);
  CHECK_GE(duration_s, 0.0);

  std::filesystem::create_directories(output_dir);

  // Serialize only to obtain body names and gravitational parameters.
  serialization::Plugin plugin_message;
  WriteToMessage(&plugin_message);
  ...
  auto write_manifest = [&]() {
    ...
    CHECK(manifest.good()) << output_dir + "/manifest.json";

    serialization::Point current_time_message; // CORREÇÃO: Point em vez de Instant
    current_time_.WriteToMessage(&current_time_message);

    manifest << std::setprecision(17);
    manifest << "{\n";
    manifest << "  \"schema\": \"principia_binary_chunks.v0_3\",\n";
    manifest << "  \"central_body\": \"" << central_body_name << "\",\n";
    manifest << "  \"central_index\": " << central_index << ",\n";
    manifest << "  \"body_count\": " << bodies.size() << ",\n";
    manifest << "  \"current_time_s\": "
             << current_time_message.scalar().magnitude() << ",\n";
    manifest << "  \"start_offset_s\": " << start_offset_s << ",\n";
    manifest << "  \"duration_s\": " << duration_s << ",\n";
    manifest << "  \"step_s\": " << step_s << ",\n";
    manifest << "  \"chunk_s\": " << chunk_s << ",\n";
    manifest << "  \"frame_transform\": \"Principia raw (X,Y,Z) -> LevelA (-Y,+Z,+X)\",\n";
    manifest << "  \"units\": \"states are meters and meters_per_second\",\n";
    manifest << "  \"bodies\": [\n";

    for (int i = 0; i < plugin_message.ephemeris().body_size(); ++i) {
      auto const& body_message = plugin_message.ephemeris().body(i);
      manifest << "    {\"index\": " << i
               << ", \"name\": \"" << body_message.name()
               << "\", \"mu_m3_s2\": "
               << body_message.gravitational_parameter().magnitude()
               << "}";
      if (i + 1 != plugin_message.ephemeris().body_size()) {
        manifest << ",";
      }
      manifest << "\n";
    }
  ...
  auto write_pod = [](std::ofstream& out, auto const& value) {
    out.write(reinterpret_cast<char const*>(&value), sizeof(value));
  };

  auto write_string = [&](std::ofstream& out, std::string const& s) {
    std::uint32_t const n = static_cast<std::uint32_t>(s.size());
    write_pod(out, n);
    out.write(s.data(), n);
  };

  auto write_chunk_file = [&](int const chunk_index,
                              double const chunk_start_offset_s,
                              double const chunk_end_offset_s) {
    CHECK_LE(chunk_start_offset_s, chunk_end_offset_s);

    std::uint64_t const sample_count =
        static_cast<std::uint64_t>(
            std::floor((chunk_end_offset_s - chunk_start_offset_s) / step_s)) + 1;

    std::vector<double> epochs(sample_count);
    std::vector<double> states(
        static_cast<std::size_t>(bodies.size()) *
        static_cast<std::size_t>(sample_count) *
        6);

    auto state_at = [&](int body_index, std::uint64_t sample_index, int component)
        -> double& {
    ...
    for (std::uint64_t sample_index = 0; sample_index < sample_count;
         ++sample_index) {
      double const offset_s =
          chunk_start_offset_s + static_cast<double>(sample_index) * step_s;

      Instant const t = current_time_ + offset_s * Second;
      ephemeris_->Prolong(t).IgnoreError();

      // 1. CORREÇÃO: O tempo é um Point
      serialization::Point t_message;
      t.WriteToMessage(&t_message);
      epochs[sample_index] = t_message.scalar().magnitude();
      ...
      std::vector<LocalState> local_states(bodies.size());
      ...
      for (int i = 0; i < bodies.size(); ++i) {
        ...
        // 2. CORREÇÃO: Extraímos Posição e Velocidade separadamente
        serialization::Point p_message;
        degrees_of_freedom.position().WriteToMessage(&p_message);
        auto const& r = p_message.multivector().vector();
        ...
        // 3. CORREÇÃO: Adicionado o .quantity() antes do .magnitude()
        transform_position(
            r.x().quantity().magnitude(), r.y().quantity().magnitude(), r.z().quantity().magnitude(),
            tx, ty, tz);

        transform_position(
            v.x().quantity().magnitude(), v.y().quantity().magnitude(), v.z().quantity().magnitude(),
            tvx, tvy, tvz);

        local_states[i] = {tx, ty, tz, tvx, tvy, tvz};
      }

      LocalState const central = local_states[central_index];

      for (int i = 0; i < bodies.size(); ++i) {
        LocalState const st = local_states[i];
        ...
        state_at(i, sample_index, 0) = x;
        state_at(i, sample_index, 1) = y;
        state_at(i, sample_index, 2) = z;
        state_at(i, sample_index, 3) = vx;
        state_at(i, sample_index, 4) = vy;
        state_at(i, sample_index, 5) = vz;
      }
    }

    std::ostringstream name;
    name << output_dir << "/chunk_"
         << std::setw(6) << std::setfill('0') << chunk_index
         << ".bin";

    std::ofstream out(name.str(), std::ios::binary);
    CHECK(out.good()) << name.str();

    char const magic[8] = {'P', 'N', 'C', 'K', 'V', '0', '3', '\0'};
    out.write(magic, sizeof(magic));

    std::uint32_t const version = 3;
    std::uint32_t const body_count = static_cast<std::uint32_t>(bodies.size());
    std::int32_t const central_index_i32 = static_cast<std::int32_t>(central_index);

    write_pod(out, version);
    write_pod(out, body_count);
    write_pod(out, sample_count);
    write_pod(out, central_index_i32);
    write_pod(out, chunk_index);
    write_pod(out, chunk_start_offset_s);
    write_pod(out, chunk_end_offset_s);
    write_pod(out, step_s);
    write_pod(out, epochs.front());
    write_pod(out, epochs.back());

    for (int i = 0; i < plugin_message.ephemeris().body_size(); ++i) {
      auto const& body_message = plugin_message.ephemeris().body(i);
      std::int32_t const body_index_i32 = i;
      double const mu = body_message.gravitational_parameter().magnitude();

      write_pod(out, body_index_i32);
      write_pod(out, mu);
      write_string(out, body_message.name());
    }

    out.write(
        reinterpret_cast<char const*>(epochs.data()),
        static_cast<std::streamsize>(epochs.size() * sizeof(double)));

    out.write(
        reinterpret_cast<char const*>(states.data()),
        static_cast<std::streamsize>(states.size() * sizeof(double)));

    LOG(INFO) << "[OK] binary chunk " << chunk_index
              << " samples=" << sample_count
              << " start_et=" << epochs.front()
              << " end_et=" << epochs.back()
              << " path=" << name.str();
  };

  int chunk_index = 0;
  double chunk_start = start_offset_s;
  double const final_offset = start_offset_s + duration_s;

  while (chunk_start <= final_offset + 1e-9) {
    double const chunk_end = std::min(chunk_start + chunk_s, final_offset);
    write_chunk_file(chunk_index, chunk_start, chunk_end);

    if (chunk_end >= final_offset) {
      break;
    }

    // Overlap endpoints between chunks. This avoids coverage gaps.
    chunk_start = chunk_end;
    ++chunk_index;
  }
}
void Plugin::DumpCurrentEphemerisSnapshotToCsv(
    std::string const& path, 
    double const time_offset_s) const { // Atualize a assinatura

  // 1. Calcular o tempo de avaliação logo no início
  Instant const evaluation_time = current_time_ + time_offset_s * Second;

  // 2. Prolongar a ephemeris até o novo tempo
  ephemeris_->Prolong(evaluation_time).IgnoreError();
  ...
  out << std::setprecision(17);
  out << "index,body,time_s,mu_m3_s2,x_m,y_m,z_m,vx_m_s,vy_m_s,vz_m_s\n";

  // 3. Extrair o valor escalar do tempo deslocado para o CSV
  serialization::Point evaluation_time_message;
  evaluation_time.WriteToMessage(&evaluation_time_message);
  auto const time_s = evaluation_time_message.scalar().magnitude();

  for (int i = 0; i < bodies.size(); ++i) {
    auto const body = bodies[i];
    auto const trajectory = ephemeris_->trajectory(body);
    
    // Evaluate no tempo correto
    auto const degrees_of_freedom =
        trajectory->EvaluateDegreesOfFreedom(evaluation_time);

    // 2. Extração da Posição
    serialization::Point p_message;
    degrees_of_freedom.position().WriteToMessage(&p_message);
    auto const& r = p_message.multivector().vector();

    // 3. Extração da Velocidade
    serialization::Multivector v_message;
    degrees_of_freedom.velocity().WriteToMessage(&v_message);
    auto const& v = v_message.vector();
    ...
    // 4. A Correção final: extraindo a magnitude da "quantity"
    out << i << ","
        << plugin_message.ephemeris().body(i).name() << ","
        << time_s << "," // Agora o tempo no CSV estará sincronizado!
        << plugin_message.ephemeris().body(i).gravitational_parameter().magnitude() << ","
        << r.x().quantity().magnitude() << ","
        << r.y().quantity().magnitude() << ","
        << r.z().quantity().magnitude() << ","
        << v.x().quantity().magnitude() << ","
        << v.y().quantity().magnitude() << ","
        << v.z().quantity().magnitude() << "\n";
  }
}
}  // namespace internal
}  // namespace _plugin
