...
class Plugin {
 public:
  ...
  virtual ~Plugin();
  // Getter injetado para o Validator Nativo
  physics::_ephemeris::Ephemeris<ksp_plugin::_frames::Barycentric>* ephemeris() const { return ephemeris_.get(); }
  ...
  // Devolve a efeméride como constante (o FlowWithAdaptiveStep permite isso!)
  Ephemeris<ksp_plugin::_frames::Barycentric> const* ephemeris_for_testing() const { 
      return ephemeris_.get(); 
  }
  
  // 2. Os parâmetros perfeitos de integração adaptativa
  Ephemeris<ksp_plugin::_frames::Barycentric>::AdaptiveStepParameters const& parameters_for_testing() const { 
      return psychohistory_parameters_; 
  }
  ...
  void DumpCurrentEphemerisSnapshotToCsv(
    std::string const& path, 
    double time_offset_s = 0.0) const;
  // Must be called after initialization.
  virtual void WriteToMessage(not_null<serialization::Plugin*> message) const;
  // TEMPORARY / OFFLINE EPHEMERIS EXPORTER.
  // Samples the reanimated Principia ephemeris from current_time_ + start_offset
  // to current_time_ + start_offset + duration, at fixed step, and writes
  // Sun-relative LevelA-frame states to CSV.
  void DumpSampledEphemerisToCsv(
      std::string const& path,
      ...
      double step_s) const;
  // TEMPORARY / OFFLINE EPHEMERIS EXPORTER.
  // Samples the reanimated Principia ephemeris and writes binary chunk files.
  // Output states are central-body-relative and in the LevelA canonical frame:
  // Principia raw (X,Y,Z) -> LevelA (-Y,+Z,+X).
  void DumpSampledEphemerisToBinaryChunks(
      std::string const& output_dir,
      std::string const& central_body_name,
      double start_offset_s,
      double duration_s,
      double step_s,
      double chunk_s) const;
